...
/* Thumbnail mode: when loaded as ?thumb=1 (dashboard cards), render the mockup full-bleed (no harness chrome). */
try{ if(/[?&]thumb\b/.test(location.search)) document.documentElement.classList.add('pmw-thumb'); }catch(e){}
/* --- comment core --- */
...
// also filters dropdowns so view seats only see Main + current branch, no dividers.
function applyViewRestrictions(role, doMenuCleanup) {
  if (doMenuCleanup === undefined) doMenuCleanup = true;
...
applyViewRestrictions((window.PMWAuth && PMWAuth.role) ? PMWAuth.role() : 'view', false);
// Auth-resolved call: applies full restrictions (including menu cleanup) with the real role
...
(function(){
  function init(){
    ...
    Promise.all([fetch('iterations.json?b='+Date.now()).then(function(r){return r.json();}),fetch(SBU+'/rest/v1/iteration_names?select=mockup,name',{headers:SBH}).then(function(r){return r.json();}).catch(function(){return [];})]).then(function(_res){
      ...
      // ===== (A) #iter-dd is the branch/screen selector — same as on Main, listing Main + all branches =====
      ...
      imenu.querySelectorAll('.pv-dd-item').forEach(function(item){item.addEventListener('click',function(){var f=item.getAttribute('data-file');idd.classList.remove('open');if(f&&f!==cur)window.location.href=f;});});
      var addB=imenu.querySelector('.iter-add');if(addB){addB.addEventListener('click',function(e){e.stopPropagation();idd.classList.remove('open');copyPrompt('Add a new iteration to the "'+_name+'" branch — duplicate the latest iteration of this branch as a new iteration, register it in iterations.json, and open it.');});}
